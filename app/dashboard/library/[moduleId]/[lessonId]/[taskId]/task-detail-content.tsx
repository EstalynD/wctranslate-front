...
/* ===== Main Component ===== */
export function TaskDetailContent({ moduleId, lessonId, taskId }: TaskDetailContentProps) {
  ...
  const [postQuizId, setPostQuizId] = useState<string | null>(null)

  const fetchData = useCallback(async (signal?: AbortSignal) => {
    if (fetchedRef.current && state.lesson) return
    ...
    try {
      ...
      // 3. Lista de todas las lecciones del tema (ordenadas por order)
  ...
  }, [moduleId, lessonId, taskId, router, state.lesson])
  ...
  // Re-fetch público (para completar tarea, etc.)
  ...
  const handleMarkComplete = useCallback(async () => {
    ...
    try {
      ...
      if (!response.success) {
        // Si requiere POST_QUIZ, abrir el modal del quiz en vez de mostrar error
        if (response.postQuizRequired?.quizId) {
          setPostQuizId(response.postQuizRequired.quizId)
  ...
  }, [state.lesson, state.lessonStatus, isCompleting, refetchData])

  // Handler cuando se completa el POST_QUIZ
  const handlePostQuizComplete = useCallback(async (passed: boolean) => {
    setShowPostQuizModal(false)
    setPostQuizId(null)

    if (passed && state.lesson) {
      // Reintentar marcar la lección como completada después de aprobar
      setIsCompleting(true)
      try {
        const response = await progressService.markLessonComplete(state.lesson._id)
        ...
        if (response.success) {
          ...
          setActionError(response.message || "No se pudo completar la lección.")
        }
      } catch {
        setActionError("No se pudo completar la lección. Intenta de nuevo.")
      } finally {
        setIsCompleting(false)
      }
    } else {
      // No aprobó el quiz, refrescar estado
      await refetchData()
      setActionError("Debes aprobar el quiz para completar esta tarea.")
    }
  }, [state.lesson, refetchData])

  // Loading / Error
  if (state.isLoading) return <TaskDetailSkeleton />
  if (state.error) return <TaskDetailError error={state.error} onRetry={handleRetry} />
  ...
  return (
    <div className="min-h-screen bg-[#0b0a1a] text-white">
        ...
        <div className="px-2 sm:px-4 md:px-6 lg:px-8 mb-4">
          <div className="flex items-start gap-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
            ...
            <div className="space-y-1">
              ...
              {contentStatus?.postQuiz?.required && contentStatus?.postQuiz?.quizId && !contentStatus?.postQuiz?.hasPassed && !isCompleted && (
                <button
                  onClick={() => {
                    setPostQuizId(contentStatus.postQuiz!.quizId!)
                    setShowPostQuizModal(true)
                  }}
                  className="mt-3 flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-orange-400 to-pink-500 text-white font-bold text-sm shadow-lg hover:shadow-orange-500/30 transition-all"
                >
                  <HelpCircle className="size-4" />
                  Iniciar Quiz de Evaluación
                </button>
              )}
            </div>
          </div>
      ...
      {/* Modal de Post-Quiz (evaluación para completar la tarea) */}
      {postQuizId && (
        <QuizModal
          isOpen={showPostQuizModal}
          onClose={() => {
            setShowPostQuizModal(false)
            setPostQuizId(null)
          }}
