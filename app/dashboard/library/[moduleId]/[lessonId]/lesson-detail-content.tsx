...
/* ===== Main Component ===== */
export function LessonDetailContent({ moduleId, lessonId }: LessonDetailContentProps) {
  ...
  const fetchData = useCallback(async (signal?: AbortSignal) => {
    if (fetchedRef.current && state.theme) return
    ...
    try {
      ...
      // 5. Obtener acceso a lecciones en batch
      ...
      if (lessonsForAccess.length > 0) {
        try {
          const lessonIds = lessonsForAccess.map((l) => l._id)
          lessonAccessMap = await progressService.getLessonsAccessBatch(lessonIds)
        } catch (err) {
  ...
  }, [moduleId, lessonId, router, state.theme])
  ...
  if (state.error) return <LessonDetailError error={state.error} onRetry={handleRetry} />
